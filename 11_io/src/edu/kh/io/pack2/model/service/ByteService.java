...
public class ByteService {
	...
	public void fileByteOutput() {
		...
		try {
			...
			// String 불변성 분제를 해결한 객체(가변성)
			StringBuilder sb = new StringBuilder();
			sb.append("Hello World!!\n");
			sb.append("12345698\n");
			sb.append("!@#$@%^^\n");
			sb.append("가나다라마바사\n");
			sb.append("와 자바 드릅게 어렵다\n");
			sb.append("샘 어려워여");
			sb.append("샘 살려주세여");
			sb.append("자바 어려웡");
			sb.append("코딩 어려웡");
...
//			}
			...
			// System.namoTime() : 1970.01.01 09:00:00 부터
			// 현재 시같까지의 차이를 ns(nano second) 단위로 반환
			// 1 ms == 1/1000초
			// 1us ( 마이크로) == 1/1000ms
			// 1ns(나노) == 1/1000us
	...
	public void bufferedFileByteOutput() {
		...
		try {
			...
			try {
				...
				// 보조 스트림 close() 시
				// 보조 스트림 생성에 사용된 기반스트림도 같이 close()됨
	...
	public void fileByteInput() {
		...
		try {
			fis = new FileInputStream("\\io_test\\20240215\\노래가사.txt"); // 인풋스트림 생성구문
	...
	public void fileByteInput2() {
		...
		try {
			...
			// 				 bytr[] 형태로 반환 받기
			//				 -> 이 후 bytr[] 배열을 이용해서 String 객체 생성
	 ...
	 * 
	 * 1) 입력된 경로에 파일이 있는지 검사 : File.existe()
	 * 
