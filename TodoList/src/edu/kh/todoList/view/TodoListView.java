...
public class TodoListView {
	...
	public TodoListView() {
		...
		// 객체 생성 시 발생되는 에외를 여기 모아서 처리할 예정
	...
	private void todoListFullView() {
		...
		for(int i=0, len = todoList.size(); i < len; i++) {
			String title = todoList.get(i).getTitle(); // list에서 i번째 얻어옴
			
			String completeYN = todoList.get(i).isComplete() ? "O" : "X"; // 삼항연산자
	...
	private void todoComplete() throws NumberFormatException, IOException {
		...
		// 서비스 호출하고 결과(tT/F) 반환 받기
		boolean result = service.todoComplete(index);
		
		if(result) { // 변경 성공
			System.out.println("[변경 되었습니다]");
			
		} else { // index요소가 범위를 초과한 경우
	...
	/**
	 * 할 일 수정
	 * - 인덱스 번호를 입력 받아 정상 범위 내의 index값인지 확인
