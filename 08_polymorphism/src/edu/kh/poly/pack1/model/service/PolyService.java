...
//		s2.setAndroidVercion(15); // 오류

		// The method setIosVersion(int) is undefined for 
		// the type SmartPhone
		// -> SmartPhone에 setIosVercion()이 정의 되지 않음(==없다)
	 ...
	 * 다향성 확인2
	 * 
	 *  (자식타입 부모 타입 참조 변수 -Down Castng, 다운 캐스팅
	 *  
	 *  - 업 캐스팅 된 객체(자식 -> 부모_를 참조하던
	 *  부모타입 참조 변수를
	 *  자식 타입으로 강제 형변환 하여
	 *  다시 자식 객체를 참조하더록 변경(부모->자식)
	 */
	public void method2() {
...
//		int v2 = s2.getAndroidVersion();
		...
		// 다운 캐스팅 + 얕은 복사(주소만 복사)
		Iphone i1 = (Iphone)s1; // s1을 Iphone으로 강제 형변활 누
														// 주소를 i1 대입
		
		Galaxy g1 = (Galaxy)s2;
		
		// 자식 객체 메서드 참조 코드 작성이 간편애짐
		i1.setIosVersion(20);
	...
	public void method5() {
		...
		// 부모 참조 변수 = 자식 객체(업캐스팅)		
		arr[2]	= new Galaxy("AMOKED", "LG", "스냅드래곤3",	14);
		...
		for(SmartPhone s : arr) { // [1], [2]번 인덱스에서 업캐스 적용ㅇ
	...
	public void printToString(SmartPhone s) {
		...
		// 실제 참조하는 객체가 자식 객체면
		// 자식 객체의 오버라이딩 된   toString() 호출(동적 바인딩)
		System.out.println(s.toString());
	}
